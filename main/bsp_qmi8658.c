...
bool bsp_qmi8658_read_acceleration_mps2(float *x, float *y, float *z)
{
    uint8_t status = 0;
    uint8_t raw[6];
    ...
    if (bsp_qmi8658_reg_read(QMI8658_STATUS0, &status, 1) != ESP_OK ||
        (status & 0x03U) == 0U ||
        bsp_qmi8658_reg_read(QMI8658_AX_L, raw, sizeof(raw)) != ESP_OK) {
        return false;
    }
    ...
    const float scale = (4.0f / 32768.0f) * STANDARD_GRAVITY_MPS2;
    *x = (float)ax * scale;
    *y = (float)ay * scale;
    *z = (float)az * scale;
    return true;
